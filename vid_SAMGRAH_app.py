...
driven video summarization in ultrasound imaging.

This program is the deployment of the app that takes lung US vidoes and output 
summarized videos with machine classification score and lung segmentations.  

The folder format to run the app is as follows.

vid_SAMGRAH : main directory
...
To run vid_SAMGRAH_app please open the working directory and enter :
...
def paginator(label, items, items_per_page=10, on_sidebar=True, Ukey='Ukey'):
    ...
    page_number = location.selectbox(label, range(n_pages), format_func=page_format_func, key = Ukey)
...
class EncoderDecoder:
    ...
    def encoderdecoder(self):
        ...
        for video_idx, video_filename in enumerate(self.video_list):
            ...
            # segmentationOutput =  torch.from_numpy(np.array(segmentationOutput)).squeeze()
            ...
            ''' (2) Single Stride Window Segment Sampling'''
            segmentSize = 5
            ...
            for i in range(len(framesLSTM)):
                if (framesLSTM[i]==1):  
                    ...
                    if self.ClassFlag:    
                        ...
                        cv2.putText(image, labellist[currPred]+'-'+str(int(video_rewards_for_train[i]*100)), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 0), 2)
                    if self.SegFlag:
                    ...
                    cv2.imwrite(tempDir+os.sep+str(video_filename)+'_'+str(i)+'.jpg',image)
                    fileList.append(tempDir+os.sep+str(video_filename)+'_'+str(i)+'.jpg')
                    currPred = bool(video_rewards_for_train[i]>0.5)                      
                    imageLocation.image(image, caption=labellist[currPred])
            ...
            idx=random.sample(range(0, nSumFrames), 9)
            fileListRand=[]
            for i in range(len(idx)):
                fileListRand.append(fileList[idx[i]])
                
            image_iterator = paginator("Select the desired page", fileListRand,items_per_page=10, on_sidebar=True, Ukey=str(video_idx))          
            indices_on_page, images_on_page = map(list, zip(*image_iterator))
            st.image(images_on_page, caption=indices_on_page,width=200)
            st.sidebar.write("Original Number of frames =",str(n_frames))
            st.sidebar.write("Number of frames in Summary =",str(nSumFrames))
            
            # st.image(fileList,caption=["test"]*len(fileList),use_column_width=True)
...
st.markdown("<h1 style='text-align: center; color: black;'>Automated Video Summarization Tool for Lung Ultrasound</h1>", unsafe_allow_html=True)
st.markdown("<h1 style='text-align: center; color: green;'>(A Computer Generated Pattern Recognition Tool - Always Correlate Clinically)</h1>", unsafe_allow_html=True)
  
st.sidebar.markdown("<h1 style='text-align: center; color: blue;'>For more details </h1>", unsafe_allow_html=True)
...
VIDEO_DIR = st.sidebar.text_input("Input Video Directory", r'data')
SUMMARY_DIR = st.sidebar.text_input("Output Summary Directory", r'summaryData')

ClassFlag=st.sidebar.checkbox("Whether Classification Label Needs to be Displayed or Not", value=False)
# ClassFlag = st.text_input("Whether Classification Label Needs to be Displayed or Not", r'False')
SegFlag=st.sidebar.checkbox("Whether Segmentation OverLay Needs to be Displayed or Not",value=False)
# SegFlag = st.text_input("Whether Segmentation OverLay Needs to be Displayed or Not", r'False')
h5Flag =st.sidebar.checkbox("Whether features needs to saved as h5 file or Not", value=False)
# h5Flag = st.text_input("Whether features needs to saved as h5 file or Not", r'False')

if h5Flag:
    h5Path = st.sidebar.text_input("Path to save encoded data in h5 format", r'encFeatsH5')
else:
    h5Path = r'encFeatsH5'
...
if st.sidebar.button ('Analyze'):    
    ...
    parser = argparse.ArgumentParser("Code for testing CAE")
    
    parser.add_argument('-s','--VIDEO_DIR', type=str, default=VIDEO_DIR, help="path to input dataset (required)")
    parser.add_argument('--SUMMARY_DIR', type=str, default=SUMMARY_DIR, help="path to output summary video")
    parser.add_argument('--h5Path', type=str, default=h5Path, help="path to encoded data")
    parser.add_argument('--IMG_SIZE', type=int, default=256, help="Image Size")
    parser.add_argument('--ClassFlag', type=bool, default=ClassFlag, help="Whether Classification Label Needs to be Displayed or Not")
    parser.add_argument('--SegFlag', type=bool, default=SegFlag, help="Whether Segmentation OverLay Needs to be Displayed or Not")
    parser.add_argument('--h5Flag', type=bool, default=h5Flag, help="Whether features needs to saved as h5 file or Not")
